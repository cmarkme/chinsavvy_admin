...
$.extend(this,parseRes.timeObj);}catch(err){if($.datepicker._get(this.inst,'debug')){console.error("Error parsing the date/time string: "+err+"\ndate/time string = "+timeString+"\ntimeFormat = "+this._defaults.timeFormat+"\ndateFormat = "+dp_dateFormat);}
return false;}
...
return date;}
return this._base_getDateDatepicker(target,noDefault);};$.datepicker._base_parseDate=$.datepicker.parseDate;$.datepicker.parseDate=function(format,value,settings){var date;try{date=this._base_parseDate(format,value,settings);}catch(err){if($.datepicker._get(this.inst,'debug')){console.error("Error parsing the date string: "+err+"\ndate string = "+value+"\ndate format = "+format);}
...
if(allPartsLen>1){return[allParts.splice(0,allPartsLen-timePartsLen).join(separator),allParts.splice(0,timePartsLen).join(separator)];}}catch(err){if($.datepicker._get(this.inst,'debug')){console.error('Could not split the date from the time. Please check the following datetimepicker options options'+"\nthrown error: "+err+"\ndateTimeString"+dateTimeString+"\ndateFormat = "+dateFormat+"\nseparator = "+timeSettings.separator+"\ntimeFormat = "+timeSettings.timeFormat+"\nampm = "+timeSettings.ampm);}
if(err.indexOf(":")>=0){var dateStringLength=dateTimeString.length-(err.length-err.indexOf(':')-2),timeString=dateTimeString.substring(dateStringLength);return[$.trim(dateTimeString.substring(0,dateStringLength)),$.trim(dateTimeString.substring(dateStringLength))];}else{throw err;}}
...
return $([startTime.get(0),endTime.get(0)]);};$.timepicker.version="1.0.5";})(jQuery);