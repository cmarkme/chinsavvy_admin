(function(){jQuery.color={};jQuery.color.make=function(r,g,b,a){var o={};o.r=r||0;o.g=g||0;o.b=b||0;o.a=a!=null?a:1;o.add=function(c,d){for(var i=0;i<c.length;++i)
...
o.r=clamp(0,parseInt(o.r),255);o.g=clamp(0,parseInt(o.g),255);o.b=clamp(0,parseInt(o.b),255);o.a=clamp(0,o.a,1);return o;};o.clone=function(){return jQuery.color.make(o.r,o.b,o.g,o.a);};return o.normalize();}
jQuery.color.extract=function(elem,css){var c;do{c=elem.css(css).toLowerCase();if(c!=''&&c!='transparent')
break;elem=elem.parent();}while(!jQuery.nodeName(elem.get(0),"body"));if(c=="rgba(0, 0, 0, 0)")
c="transparent";return jQuery.color.parse(c);}
jQuery.color.parse=function(str){var res,m=jQuery.color.make;if(res=/rgb\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*\)/.exec(str))
...
return m(parseInt(res[1]+res[1],16),parseInt(res[2]+res[2],16),parseInt(res[3]+res[3],16));var name=jQuery.trim(str).toLowerCase();if(name=="transparent")
return m(255,255,255,0);else{res=lookupColors[name];return m(res[0],res[1],res[2]);}}
