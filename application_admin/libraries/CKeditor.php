...
class CKEditor
{
    ...
    function __construct($config = array()) {
        $ci = get_instance();
        $this->basePath = $config['ckeditor_basePath'];
        $this->config['toolbar'] = $config['ckeditor_toolbar'];
        $this->config['extraPlugins'] = $config['ckeditor_extraPlugins'];
        $ci->db->select('signature');
        $staff = $ci->user_model->get($ci->session->userdata('user_id'));
