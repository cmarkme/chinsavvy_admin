...
//            echo "$fname\n\n<br><br>\n\n";
            ...
            $CI->upload->file_name = $this->prep_filename($_FILES[$field]['name'][$i]);
...
//                echo $error_hold[$i];
                ...
                $image_data = array(
                            ...
                            'file_type' => $CI->upload->file_type,
                            'file_path' => $CI->upload->upload_path,
                            'full_path' => $CI->upload->upload_path.$CI->upload->file_name,
                            'raw_name'	=> str_replace($CI->upload->file_ext, '', $CI->upload->file_name),
                            'orig_name'	=> $CI->upload->orig_name,
                            'client_name' => $CI->upload->client_name,
                            'file_ext'	=> $CI->upload->file_ext,
                            'file_size'	=> $CI->upload->file_size);

                if ($imageVar = $this->multiple_image_properties($CI->upload->upload_path.$CI->upload->file_name)) {

                    $image_data += array(
                            'image_type' => $imageVar->image_type,
                            'image_height' => $imageVar->image_height,
                            'image_width' => $imageVar->image_width,
                            'image_size_str' => $imageVar->image_size_str
                            );
                }
                $file_list[] = $image_data;
            }
    ...
    function multiple_image_properties($path = '')
    {
        ...
        if (function_exists('getimagesize'))
        {
            if (FALSE !== ($D = @getimagesize($path)))
            {
                ...
                $image->image_width       = $D['0'];
                $image->image_height      = $D['1'];
                $image->image_type        = ( ! isset($types[$D['2']])) ? 'unknown' : $types[$D['2']];
				$image->image_size_str	= $D['3'];  // string containing height and width
